...
OrbitingBalls::OrbitingBalls(int nrOfBalls, float OrbitRadius, sf::Vector2f HostPosition, sf::RenderWindow* fönster) {
	...
	for (float r = 0.0; r < 2 * PI; r += ballPos) {
		EvilCircle* boll = new EvilCircle({ this->HostPosition.x + OrbitRadius * std::cos(r), this->HostPosition.x + OrbitRadius * std::sin(r) }, sf::Color::Yellow, OrbitRadius / 5, {this->Evil_Velocity});
		orb klot = { boll,r };
